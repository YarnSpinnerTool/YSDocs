...
| On Command                    | A Unity Event that's fired when a Command is encountered. This will only be called if no other part of the system has already handled the command, such as command handlers registered via [YarnCommand](../creating-commands-functions.md#the-yarncommand-attribute) or [AddCommandHandler](../creating-commands-functions.md#adding-commands-through-code). |
