...
Line Providers are responsible for fetching the appropriate assets for a given line and language. For example, the [Builtin Localisation Line Provider](/docs/api/csharp/yarn.unity.builtinlocalisedlineprovider.md) fetches assets from a Yarn Project, and provides them to voice-over dialogue views.
