...
| On Unhandled Command                    | A Unity Event that's fired when a Command is encountered. This will only be called if no other part of the system has already handled the command, such as command handlers registered via [YarnCommand](/docs/api/csharp/yarn.unity.yarncommandattribute.md) or [AddCommandHandler](/docs/api/csharp/yarn.unity.actions.addcommandhandler-2.md). |
